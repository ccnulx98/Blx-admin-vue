...
<script>
import { listCategory, getCategory, delCategory, addCategory, updateCategory, updateCategoryStatus, exportCategory } from '@/api/content/category'
...
export default {
  ...
  methods: {
    ...
    handleStatusChange(row) {
      const ids = row.id
      const data = { 'id': ids, 'status': row.status }
      updateCategoryStatus(data).then(response => {
        this.$modal.msgSuccess('修改成功')
      })
    },
    /** 导出按钮操作 */
